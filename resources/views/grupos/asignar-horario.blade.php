...
@section('content')
    ...
    (function () {
        ...
        const docentesList = {!! json_encode($docentes->map(fn($d) => ['id' => $d->id_docente, 'nombre' => $d->persona?->nombre_completo, 'registro' => $d->persona?->credencial?->registro])) !!};
        ...
        const docentesSelect = (name, selectedId) =>
            '<select name="' + name + '" class="docente-select" data-validate="docente" required style="width:100%;">' +
                '<option value="">Seleccionar docente</option>' +
                docentesList.map(d => '<option value="' + d.id + '">' + d.nombre + ' — ' + (d.registro || 'N/A') + '</option>').join('') +
        ...
        plantillaSelect.addEventListener('change', function () {
            ...
            docentesGrid.innerHTML = materiasUnicas.map(d =>
                '<div style="background:#fff; border:1px solid #e2e8f0; border-radius:8px; padding:14px;">' +
                    ...
                    docentesSelect('docentes[' + d.id_materia + ']', '') +
                '</div>'
