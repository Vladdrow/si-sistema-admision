...
class DocenteController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if ($isAsync) {
            return view('personas.docentes.partials.table', compact('docentes'));
        }

        $materias = Materia::orderBy('nombre')->get();
        $titulos = self::TITULOS_PROFESIONALES;

        return view('personas.docentes.index', compact('docentes', 'search', 'degree', 'status', 'materias', 'titulos'));
    }
